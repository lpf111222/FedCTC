        ...
        for e in range(self.conf["local_epochs"]):
            ...
            for batch_id, batch in enumerate(self.train_data_loader):
                ...
                lam = torch.from_numpy(
                    np.random.beta(self.conf["mixup_alpha"], self.conf["mixup_alpha"], batch_size)).to(
                    torch.float32).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
                for i in range(len(data.shape) - 1):
                    lam.unsqueeze_(i + 1)
                shuffled_index = torch.randperm(batch_size).to(
                ...
                # mix the smooth lable
                mixed_targets = lam.view(batch_size, -1) * target_smooth + (1 - lam).view(batch_size, -1) * target_smooth[shuffled_index]
                # loss
...
class FedCL_3In1_Server(server_base.ServerBase):
    def __init__(self, conf, train_dataset, eval_dataset, clients_distribution, train_clients_index, eval_clients_index, MonteCarlo_times=1):
        ...
        for i in range(conf["num_client"]):
            self.clients.append(
                FedCL_3In1_Client(self.global_model, conf, train_dataset, eval_dataset, clients_distribution[i],
